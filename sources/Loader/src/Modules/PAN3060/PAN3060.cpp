...
void PAN3060::CallbackOnIRQ()
{
    ...
    if (irq & REG_IRQ_RX_DONE)
    {
        ...
        if (packet.length == 2 + 128 + 4)            // ѕринимаем 128 байт прошивки
        {
            rf_init();
            rf_set_default_para();
            rf_enter_continous_rx();
        ...
        else if (packet.length == 2 + 4 + 4)         // ѕринимаем завершающий пакет
        {
            rf_init();
            rf_set_default_para();
            rf_enter_continous_rx();
...
void PAN3060::Packet::ReceiveChain()
{
    ...
    if (number_chain_in_page == 0)          // ѕервый чайн в странице, готовимс€ к приЄму новой страницы
    {
        page.Clear();
