...
void myDisplay_ui_firstUi(bool flashFlag, int agr0, void *agr1_ptr, void *agr2_ptr)
{
    ...
    {
        myLCD_16x16(IM_INVERSE, 10, 11, "ÉîÛÚÊÐÎÖ½ø¿Æ¼¼ÓÐÏÞ¹«Ë¾");
...
void myDisplay_ui_upgradeFirmware(bool flashFlag, int agr0, void *arg1_ptr, void *arg2_ptr)
{
    ...
    myLCD_str8x16(IM_NOMALE, 0, 5, "%d sec" , value);

    myLCD_str8x16(IM_NOMALE, 0, 6, "%d/%d chains, %.1f %%", upg_chains_transmitted(), upg_chains_all(), (upg_chains_transmitted() / upg_chains_all()) * 100.0f);
}
