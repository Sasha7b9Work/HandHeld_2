...
/*
    ѕрошивку в двоичном виде размером ровно 54 * 1024 байт нужно разместить по адресу 0x08030000

    ќбновление происходит так.
    1. ¬с€ прошивка 54 кЅ делитс€ на куски 128, которыми и будем передавать.  аждый кусок передаЄтс€ в таком виде:
        2 байта     номер куска
        128 байт    кусок
...
static const uint8_t *data = (const uint8_t *)DATA_BEGIN;   // Ёто указатель на данные, которые следует передавать в данной итерации
#define SIZE_CHAIN 128

//----------------------------------------------------------------------------------------------------------------------------------

// —брасывает указатель данных прошивки
static void Reset(void);

// ¬озвращает указатель на следующую порцию данных.
// ≈сли 0 - данные закончились
...
// «аслать пакет непосредственно в передатчик
...
static uint32_t time_big = 60;
static uint32_t time_small = 60;
...
static uint32_t ProcessCRC(const uint8_t *data, int length, uint32_t crc)
{
    static const uint32_t tableCRC[256] =
    {
    ...
    while (length--)
    {
        crc = tableCRC[(uint8_t)(crc) ^ *data++] ^ (crc >> 8);
    }
