...
// Структура разбивает на тройки выводимое значение
struct Splitter
{
    Splitter(char *text, bool splitThrees) : counter(0), split(splitThrees)
    {
        ...
        counter = (int)(numSymbols % 3);        // Это значение будем увеличивать после вывода каждого символа.
                                                // Когда значение станет равно трём, обнуляем его и делаем промежуток
...
uint FontBig::WriteSymbol(uint8 symbol, int x, int y, bool mapping)
{
    if (symbol == '-')
    {
        if (mapping)
        {
            Rect(BIGSPACE_WIDTH, 7).Fill(x, y + 20);
        }
