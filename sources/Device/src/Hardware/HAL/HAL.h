...
namespace HAL
{
    ...
    // Возвращает true, если это "отладочноая" плата, на которой ведётся разработка.
    // На отладочной плате при подключении отладчика неправильно определяет напряжение
    bool IsDebugBoard();
}
